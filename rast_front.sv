...
module rast_front #(
    ...
    //   FB_BASE      0x30000
    input wire [MEM_AW-1:0] tri_base,
    input wire [MEM_AW-1:0] bin_base,
    input wire [MEM_AW-1:0] binlist_base,
    ...
    wire [MEM_AW-1:0] bin_full  = bin_base + MEM_AW'(tile_idx);
    wire [MEM_AW-1:0] tidx_full = binlist_base
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            case (state)
                ...
                S_FETCH_TRI: begin
                    if (mstate == M_IDLE && !burst_go && !burst_ack) begin
                        burst_addr <= tri_base + (MEM_AW'(tri_id) << 4);
                        burst_we   <= 1'b0;
                        burst_go   <= 1'b1;
                    end else if (burst_ack) begin
                ...
                S_FETCH_TRI2: begin
                    if (mstate == M_IDLE && !burst_go && !burst_ack) begin
                        burst_addr <= tri_base + (MEM_AW'(tri_id) << 4) + MEM_AW'(BURST);
                        burst_we   <= 1'b0;
                        burst_go   <= 1'b1;
                    end else if (burst_ack) begin
