...
module arbiter #(
    ...
    input  wire  [MEM_AW-1:0] m3_addr,
    input  wire               m3_req,
    input  wire               m3_we,
    input  wire  [15:0]       m3_wr_data,
    output logic              m3_wr_data_req,
    output logic [15:0]       m3_rd_data,
    output logic              m3_rd_valid,
    output logic              m3_ready,
    ...
    logic        locked;
    logic [1:0]  lock_owner;   // 0=m0, 1=m1, 2=m2, 3=m3
    wire         accept = s_req && s_ready;

    // Fresh-request selection: strict priority m0 > m1 > m2 > m3.
    wire [1:0] new_sel = m0_req ? 2'd0 : m1_req ? 2'd1 : m2_req ? 2'd2 : 2'd3;
    ...
    always_comb begin
        case (sel)
            ...
            2'd2:    begin s_addr = m2_addr; s_we = m2_we; s_wr_data = m2_wr_data; end
            default: begin s_addr = m3_addr; s_we = m3_we; s_wr_data = m3_wr_data; end
        endcase
    end
    assign s_req = locked ? 1'b0 : (m0_req | m1_req | m2_req | m3_req);
    ...
    assign m3_ready = !locked && !m0_req && !m1_req && !m2_req && s_ready;
    ...
    assign m3_wr_data_req = s_wr_data_req && (sel == 2'd3);
